...
#Koordinat Lokasi
inLL = -6.928895 #Lintang lokasi
LL = float (inLL)
LLR = radians(LL)

inBL = 107.716743 #Bujur lokasi
BL = float (inBL)
...
#Data lainnya
inKL = 720 #Ketinggian lokasi (meter)
KL = float (inKL)
inTZ = 7 #Time Zone (GMT)
TZ = float (inTZ)
print("")

#Ikhtiyah
IS = 2 #Ikhtiyah Awal Subuh (menit)
print (" Ikhtiyah Awal Subuh (menit)       = ", IS)
IaS = -2 #Ikhtiyah Akhir Subuh (menit)
print (" Ikhtiyah Akhir Subuh (menit)      = ", IaS)
IDh = 2 #Ikhtiyah Dhuha (menit)
print (" Ikhtiyah Awal Dhuha (menit)       = ", IDh)
IDz = 4 #Ikhtiyah Dzuhur (menit)
print (" Ikhtiyah Awal Dzuhur (menit)      = ", IDz)
IA = 2 #Ikhtiyah Ashar (menit)
print (" Ikhtiyah Awal Ashar (menit)       = ", IA)
IM = 2 #Ikhtiyah Maghrib (menit)
print (" Ikhtiyah Awal Maghrib (menit)     = ", IM)
II = 2 #Ikhtiyah Isya (menit)
print (" Ikhtiyah Awal Isya (menit)        = ", II)

WIm = 10    #Selisih Imsak dengan subuh
SS = 20     #Sudut subuh
SDh = 4.5   #Sudut Dhuha
SI = 18     #Sudut Isya

"""
Kriteria Penghitungan Waktu Sholat 
Penghitungan waktu sholat untuk wilayah Indonesia
(dan juga negara-negara Asia Tenggara - MABIMS) 
dilakukan berdasarkan kriteria Kementerian Agama (Depag), 
Republik Indonesia. Kriteria Kemenag (MABIMS) adalah: 
Waktu Imsak -10 menit dari waktu Shubuh 
Sudut Shubuh -20°
Sudut Dhuha 4.5° 
Zhuhur setelah matahari melintas Meridian (Zawal) 
Penentuan 'Ashar berdasarkan Figih Syafi'i: panjang bayangan - panjang benda 
Maghrib saat terbenam matahari 
Sudut 'Isya' -18°

Selain itu, waktu sholat pada jadwal ini juga dikoreksi dengan Ikhtiyat +- 2 menit 
(sebagai waktu pengaman dan penyeragam agar bisa berlaku bagi seluruh wilayah kota).
"""

print('+====================================================+')
print(" Tgl " + " Imsak " + " Terbit " + " Dhuha " +\
...
for H in range (H,Ha+1):
    ...
    if MI_WDz<10:
        ...
        MI_WDz = MI_WDz
    #--------------------------#
    WA = TM+(12/pi)*acos((sin(atan(1/(1+tan(abs(LLR-DMR)))))-sin(DMR)*sin(LLR))/(cos(DMR)*cos(LLR)))
    WA = WA+(IA/60)+(1/60)
    ...
    if MI_WA<10:
        ...
        MI_WA = MI_WA
    #--------------------------#
    TbM=TM+(12/pi)*acos((sin((-0.8333-0.00347*KL**(1/2))*pi/180)-sin(DMR)*sin(LLR))/(cos(DMR)*cos(LLR)))
    TbM = TbM+(0.5/60)
    #print ("Terbenam Matahari         = ", HA)
    D_TbM = TbM
    ...
    #------------------------#
    Im = WS-(WIm/60)
    #print ("Waktu Imsyak Desimal       =", WIm)
    D_Im = Im
